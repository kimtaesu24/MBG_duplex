...
FUSION_PARAMETER_NAMES = (
    "backchannel_fusion_bc_weight",
    "backchannel_fusion_vap_weight",
    "backchannel_fusion_vad_weight",
    "backchannel_fusion_bias",
)


def fusion_state(lm) -> dict:
    """Return the effective fusion calibration used by forward/LMGen."""
    state = {
        "trainable": bool(getattr(lm, "backchannel_fusion_trainable", False)),
    }
    for name in FUSION_PARAMETER_NAMES:
        value = getattr(lm, name, None)
        state[name.removeprefix("backchannel_fusion_")] = (
            float(value.detach().float().cpu().item()) if value is not None else None
        )
    return state


def log_fusion_state(lm) -> dict:
    """Log and return checkpoint-backed fusion parameters."""
    state = fusion_state(lm)
    if getattr(lm, "backchannel", None) is None:
        log("info", "Backchannel and learned fusion are disabled.")
    elif state["trainable"]:
        log(
            "info",
            "Checkpoint-trained fusion "
            f"bc={state['bc_weight']:.6f}, vap={state['vap_weight']:.6f}, "
            f"vad={state['vad_weight']:.6f}, bias={state['bias']:.6f}",
        )
    else:
        log("info", "Fusion is not trainable.")
    return state
...
def load_checkpoint(
    ...
    if os.path.exists(consolidated_path):
        ...
        if getattr(lm, "backchannel_fusion_trainable", False):
            missing_fusion = [name for name in FUSION_PARAMETER_NAMES if name not in state_dict]
            if missing_fusion:
                raise RuntimeError(
                    "fusion_trainable=True, but checkpoint is missing learned fusion "
                    f"parameters: {missing_fusion}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="./output/example_0806/args.yaml", help="Path to args.yaml or training config yaml")
    parser.add_argument(
        "--no-auto-fix-config-paths",
        action="store_true",
        help="Disable automatic rebasing of missing paths copied from another server.",
    )
    parser.add_argument("--test-jsonl", type=str, default='./ami_test_10.jsonl', help="Path to data.jsonl for the test dataset")
    parser.add_argument("--output-dir", type=str, default="./result/example_0806/checkpoint_000500", help="Directory to save generated outputs")
    parser.add_argument("--ckpt-dir", type=str, default="./output/example_0806/checkpoints/checkpoint_000500", help="Directory containing consolidated/lora.safetensors")
    parser.add_argument(
        "--base-model-only",
        action="store_true",
