...
def _strip_peft_prefixes(state_dict: dict) -> dict:
    """Strip PEFT/training wrapper prefixes so keys match the bare LMModel namespace.

    Saved keys look like:
      base_model.model.model.<lm_key>          (LoRA weights or backchannel)
      base_model.model.model.<lm_key>.original_module.*  (skip: frozen original)
    After stripping → <lm_key>
    """
    cleaned = {}
...
def load_checkpoint(lm, ckpt_dir):
    ...
    if os.path.exists(consolidated_path):
        ...
        bc_weights = direct_weights  # keep alias for legacy block below
