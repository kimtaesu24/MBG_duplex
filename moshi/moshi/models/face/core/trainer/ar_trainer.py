...
class ARTrainer(BaseTrainer):
    ...
    def _calc_metrics(self, infer_results, visualize=None):
        if infer_results["pred_motion_code"].shape[-1] != 108:
            gt_motion_code = infer_results["gt_motion_code"]
            pred_motion_code = infer_results["pred_motion_code"]
            l1 = torch.nn.functional.l1_loss(pred_motion_code, gt_motion_code)
            pred_vel = pred_motion_code[:, 1:] - pred_motion_code[:, :-1]
            gt_vel = gt_motion_code[:, 1:] - gt_motion_code[:, :-1]
            vel = torch.nn.functional.mse_loss(pred_vel, gt_vel)
            return {"L1": l1, "VEL": vel}
