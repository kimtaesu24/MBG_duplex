...
class BaseTrainer:
    ...
    def run_validation(self, iter_idx, stage="val"):
        ...
        for idx, batch_data in enumerate(val_bar):
            ...
            if self.accelerator.is_main_process:
                self._save_validation_motion(iter_idx, stage, idx, infer_results, batch_data)
    ...
    def _save_validation_motion(self, iter_idx, stage, idx, infer_results, batch_data):
        if not hasattr(self, "_dump_dir"):
            return
        pred_motion = infer_results.get("pred_motion_code")
        gt_motion = infer_results.get("gt_motion_code")
        if pred_motion is None:
            return
        info = batch_data.get("infos", {})
        source_id = f"sample_{idx:04d}"
        ...
        out_dir = Path(self._dump_dir) / "val_flame" / f"iter_{iter_idx}" / stage
        out_dir.mkdir(parents=True, exist_ok=True)
        np.save(out_dir / f"{safe_id}_pred.npy", self._motion_for_save(pred_motion[0]))
        if gt_motion is not None:
            np.save(out_dir / f"{safe_id}_gt.npy", self._motion_for_save(gt_motion[0]))

    @staticmethod
    def _motion_for_save(motion):
