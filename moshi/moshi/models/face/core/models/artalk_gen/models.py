...
class ARTalkGen(nn.Module):
    def __init__(self, model_cfg=None, init_submodule=True, **kwargs):
        ...
        # build basic vae
        base_codec = ARTalkCodec(model_cfg.VAE_CONFIG, init_submodule=False)
        if init_submodule:
        ...
        # build audio encoder
        self.uses_cached_audio_feat = model_cfg.AUDIO_ENCODER in ("cached_mimi", "mimi_cached", "precomputed_mimi")
        ...
        elif self.uses_cached_audio_feat:
            audio_dim = 512
            audio_encoder = nn.Identity()
        else:
    ...
    def forward(self, batch, training=True):
        ...
        if "audio_feat" in batch:
            ...
            audio_feat = self.audio_encoder(audio, self._sample_rate)

        # mask conditions for classifier-free training
        m_audio = torch.rand(batch_size, 1, 1).to(self.device) < self.audio_free_ratio
        ...
        audio_feat = torch.where(m_audio, torch.zeros_like(audio_feat), audio_feat)
        style_motion_code = torch.where(m_style, torch.zeros_like(style_motion_code), style_motion_code)
        prev_motion_code = torch.where(m_prev, torch.zeros_like(prev_motion_code), prev_motion_code)
        ...
        attn_feat = self.attn_blocks(attn_feat, audio_feat, prev_feat, style_feat)
    ...
    def inference(self, audio=None, audio_feat=None, style_motion_code=None, prev_motion_code=None, tau=1.0, cfg=2.0, **kwargs):
        ...
        if audio_feat is not None:
            if "motion_code" in kwargs:
                frame_length = kwargs["motion_code"].shape[1]
            else:
                frame_length = int(audio_feat.shape[1] * 2)
            frame_chunk_length = math.ceil(frame_length / patch_len) * patch_len
            feat_chunk_size = max(1, math.ceil(patch_len / 2))
            feat_chunk_length = math.ceil(audio_feat.shape[1] / feat_chunk_size) * feat_chunk_size
            audio_feat = torch.cat(
                [audio_feat, audio_feat.new_zeros(batch_size, feat_chunk_length - audio_feat.shape[1], audio_feat.shape[2])],
                dim=1,
            )
            audio_feats = list(audio_feat.split(feat_chunk_size, dim=1))
            if audio is None:
                audio = audio_feat.new_zeros(batch_size, int(frame_length * audio_frame_len))
        else:
            ...
            audio_chunks = torch.cat(
                [audio, audio.new_zeros(batch_size, audio_chunk_length - audio.shape[-1])], dim=-1
            ).split(audio_chunk_size, dim=-1)
            audio_feats = [self.audio_encoder(ac) for ac in audio_chunks]
        ...
        # run audio chunks
        pred_motion_codes = []
        for chunk_idx in range(len(audio_feats)):
            next_ar_vqfeat = sos_token
            curr_audio_feat = audio_feats[chunk_idx]
            ...
            for pidx, pn in enumerate(self.patch_nums):
                attn_feat = self.attn_blocks(
                    next_ar_vqfeat, curr_audio_feat, prev_feat, style_feat
                )
