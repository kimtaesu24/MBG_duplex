...
class ARTalkCodec(nn.Module):
    def __init__(self, model_cfg, init_submodule=None, **kwargs):
        ...
        # for training parts
        if init_submodule and self.motion_dim == 108:
            from core.libs.flame_model import FLAMEModel  # lazy: only needed for 108-D mesh losses
    ...
    def _calc_losses(self, train_results, _loss_kwargs):
        ...
        if gt_motion_code.shape[-1] >= 108:
            gt_head_pose, pred_head_pose = gt_motion_code[..., 100:104], pred_motion_code[..., 100:104]
        elif gt_motion_code.shape[-1] == 56:
            gt_head_pose, pred_head_pose = gt_motion_code[..., 53:56], pred_motion_code[..., 53:56]
        else:
            gt_head_pose = gt_motion_code[..., -3:]
            pred_head_pose = pred_motion_code[..., -3:]
        # rec loss
        ...
        # The bundled FLAMEModel helper is hard-coded for ARTalk's 108D TKH1K
        # motion layout.  Our Mimi/DualTalk/CANDOR adapter uses 56D
        # expr50+jaw3+neck3, so mesh losses must be disabled there.
        if gt_motion_code.shape[-1] == 108 and (
            _loss_kwargs.MESH_WEIGHT
            or _loss_kwargs.LIPS_WEIGHT
            or _loss_kwargs.MESH_VEL_WEIGHT
            or _loss_kwargs.MESH_SMOOTH_WEIGHT
        ):
            from core.models.modules import calc_mesh_loss  # lazy: only for 108-D mesh losses
